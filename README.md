...
Derive it before coding it — the maths is in the comments, not abstracted away in a dependency. Test against ground truth — I know the right answer because I generated the data, so the tests check the code actually finds it. Never look ahead — causal stats, walk-forward evaluation, and tests that break if I cheat. And treat the bad results as results — a pair that loses money and gets correctly diagnosed as a regime break is more useful than three suspiciously clean Sharpe ratios.

## What it doesn't do yet (the honest list)

It's all synthetic so far — real data adapters are the obvious next step. The SVI fit is done one maturity at a time, so calendar arbitrage gets checked but not prevented (the proper fix is SSVI). The three systems are designed to hand off to each other but aren't wired into a single runner yet — that's the next thing I want to build. And there's a whole shopping list of models I'd like to add: local vol, Heston, Longstaff-Schwartz for American options.
